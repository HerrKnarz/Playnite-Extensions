...
namespace KNARZhelper.DatabaseObjectTypes
{
    public class TypeGenre : BaseType
    {
        ...
        public override bool DbObjectInUse(List<Game> games, Guid id) => games.Any(x => x.GenreIds?.Contains(id) ?? false);
        ...
        public override int GetGameCount(List<Game> games, Guid id, bool ignoreHidden = false) =>
            games.Count(g => !(ignoreHidden && g.Hidden) && (g.GenreIds?.Contains(id) ?? false));
        ...
        private void ItemUpdated(object sender, ItemUpdatedEventArgs<Genre> args)
        {
            ...
            foreach (ItemUpdateEvent<Genre> item in args.UpdatedItems.Where(item => item.OldData != null && item.OldData.Name != item.NewData.Name))
            {
