...
namespace KNARZhelper.DatabaseObjectTypes
{
    public class TypeTag : BaseType
    {
        ...
        public override int Count => API.Instance.Database.Tags?.Count ?? 0;
        ...
        public override bool DbObjectExists(Guid id) => API.Instance.Database.Tags?.Any(x => x.Id == id) ?? false;
        ...
        public override bool DbObjectInUse(List<Game> games, Guid id) => games.Any(x => x.TagIds?.Contains(id) ?? false);

        public override void EmptyFieldInGame(Game game) => API.Instance.MainView.UIDispatcher.Invoke(() => game.TagIds?.Clear());

        public override bool FieldInGameIsEmpty(Game game) => (game.TagIds?.Count ?? 0) == 0;
        ...
        public override int GetGameCount(List<Game> games, Guid id, bool ignoreHidden = false) =>
            games.Count(g => !(ignoreHidden && g.Hidden) && (g.TagIds?.Contains(id) ?? false));

        public override List<Game> GetGames(Guid id, bool ignoreHidden = false) =>
            API.Instance.Database.Games.Where(g => !(ignoreHidden && g.Hidden) && (g.TagIds?.Contains(id) ?? false)).ToList();
        ...
        public override List<DatabaseObject> LoadGameMetadata(Game game) =>
            game.Tags?.Select(x => new DatabaseObject() { Name = x.Name, Id = x.Id }).ToList() ?? new List<DatabaseObject>();
        ...
        public override bool RemoveDbObject(Guid id, bool checkIfUsed = true) =>
            ...
            DbObjectExists(id) && (checkIfUsed
                ...
                : API.Instance.MainView.UIDispatcher.Invoke(() => API.Instance.Database.Tags?.Remove(id) ?? false));
        ...
        private void ItemUpdated(object sender, ItemUpdatedEventArgs<Tag> args)
        {
            ...
            foreach (ItemUpdateEvent<Tag> item in args.UpdatedItems.Where(item => item.OldData != null && item.OldData.Name != item.NewData.Name))
            {
