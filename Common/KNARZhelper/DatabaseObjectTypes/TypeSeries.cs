...
namespace KNARZhelper.DatabaseObjectTypes
{
    public class TypeSeries : BaseType
    {
        ...
        public override bool DbObjectExists(Guid id) => API.Instance.Database.Series?.Any(x => x.Id == id) ?? false;

        public override bool DbObjectInGame(Game game, Guid id) => game.SeriesIds?.Contains(id) ?? false;

        public override bool DbObjectInUse(Guid id) => API.Instance.Database.Games.Any(x => x.SeriesIds?.Contains(id) ?? false);

        public override bool DbObjectInUse(List<Game> games, Guid id) => games.Any(x => x.SeriesIds?.Contains(id) ?? false);

        public override void EmptyFieldInGame(Game game) => API.Instance.MainView.UIDispatcher.Invoke(() => game.SeriesIds?.Clear());

        public override bool FieldInGameIsEmpty(Game game) => (game.SeriesIds?.Count ?? 0) == 0;

        public override Guid GetDbObjectId(string name) =>
            API.Instance.Database.Series?.FirstOrDefault(x => x.Name == name)?.Id ?? Guid.Empty;

        public override int GetGameCount(Guid id, bool ignoreHidden = false) =>
            API.Instance.Database.Games.Count(g => !(ignoreHidden && g.Hidden) && (g.SeriesIds?.Contains(id) ?? false));

        public override int GetGameCount(List<Game> games, Guid id, bool ignoreHidden = false) =>
            games.Count(g => !(ignoreHidden && g.Hidden) && (g.SeriesIds?.Contains(id) ?? false));

        public override List<Game> GetGames(Guid id, bool ignoreHidden = false) =>
            API.Instance.Database.Games.Where(g => !(ignoreHidden && g.Hidden) && (g.SeriesIds?.Contains(id) ?? false)).ToList();
        ...
        public override bool NameExists(string name, Guid id) =>
            API.Instance.Database.Series?.Any(x => x.Name == name && x.Id != id) ?? false;
        ...
        public override bool RemoveDbObject(Guid id, bool checkIfUsed = true) =>
            ...
            DbObjectExists(id) && (checkIfUsed
                ...
                : API.Instance.MainView.UIDispatcher.Invoke(() => API.Instance.Database.Series?.Remove(id) ?? false));

        public override bool RemoveObjectFromGame(Game game, List<Guid> ids) => ids.Count != 0 && ids.Aggregate(false, (current, id) =>
                current | API.Instance.MainView.UIDispatcher.Invoke(() => game.SeriesIds?.Remove(id) ?? false));

        public override bool RemoveObjectFromGame(Game game, Guid id) =>
            API.Instance.MainView.UIDispatcher.Invoke(() => game.SeriesIds?.Remove(id) ?? false);
        ...
        {
            Series item = API.Instance.Database.Series?.FirstOrDefault(x => x.Id == id);
