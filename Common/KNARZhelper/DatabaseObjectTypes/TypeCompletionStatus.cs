...
namespace KNARZhelper.DatabaseObjectTypes
{
    public class TypeCompletionStatus : BaseType
    {
        ...
        public override int Count => API.Instance.Database.CompletionStatuses?.Count ?? 0;
        ...
        public override bool DbObjectInUse(List<Game> games, Guid id) => games.Any(x => x.CompletionStatusId == id);
        ...
        public override int GetGameCount(List<Game> games, Guid id, bool ignoreHidden = false) =>
            games.Count(g => !(ignoreHidden && g.Hidden) && (g.CompletionStatusId == id));
        ...
        private void ItemUpdated(object sender, ItemUpdatedEventArgs<CompletionStatus> args)
        {
            ...
            foreach (ItemUpdateEvent<CompletionStatus> item in args.UpdatedItems.Where(item => item.OldData != null && item.OldData.Name != item.NewData.Name))
            {
