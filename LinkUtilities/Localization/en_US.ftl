...
settings_bookmarklet_description = Using a bookmarklet you can add the active website in your browser directly to the selected games in Playnite. To find a suiting link name, you can define patterns for the URL and link title here. The patterns can contain wildcards. A * can be zero or more characters, a ? has to be exactly one character.
...
settings_configure_link_name_patterns = Configure link name patterns (For clipboard links only the url pattern will be checked to find a name, since we have no name to begin with.)
...
settings_link_source = Link source
settings_name_pattern = Name pattern
settings_partial_match = Partial match
settings_partial_match_hint = When checked only one of both patterns has to match.
settings_search_link = Search link
settings_show_in_menus = Show in menu
settings_tab_bookmarklet_clipboard = Bookmarklet/Clipboard
