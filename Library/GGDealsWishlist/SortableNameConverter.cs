...
namespace GGDealsWishlist
{
    public class SortableNameConverter
    {
        ...
        /// </summary>
        private static readonly int numberLength = 2;

        //Haven't observed game titles with zero, or four and above that would benefit from making those words sortable numbers
        private static readonly Dictionary<string, int> numberWordValues = new Dictionary<string, int>(StringComparer.InvariantCultureIgnoreCase) { { "one", 1 }, { "two", 2 }, { "three", 3 } };
        ...
        /// </summary>
        /// <param name="articles">
        /// Words to remove from the start of the title. Suggested: the contents of
        /// PlayniteSettings.GameSortingNameRemovedArticles, or "The", "A", "An".
        /// </param>
        /// <param name="batchOperation">
        /// Optimize for larger amounts of throughput. Slower for small amounts.
        /// </param>
        public SortableNameConverter(IEnumerable<string> articles = null, bool batchOperation = false)
        {
        ...
        /// <returns>
        /// An integer form of the supplied roman numeral, or NULL if the supplied roman numeral is
        /// invalid and <paramref name="validate"/> is true.
        /// </returns>
        ...
        public static int? ConvertRomanNumeralToInt(string input, bool validate = true)
        {
            ...
            var output = 0;
            var biggestNumberToTheRight = 0;

            var prevCharGroupLength = 0;
            var lastNumericValue = 0;
            for (var i = input.Length - 1; i >= 0; i--)
            {
                var c = input[i];
                if (!romanNumeralValues.TryGetValue(c, out var value))
                {
                    return null;
                }

                var subtract = value < biggestNumberToTheRight;
                if (subtract)
                {
                    output -= value;
                }
        ...
        public string Convert(string input)
        {
            ...
            input = StripEdition(input, out var edition);
            ...
            var output = regex.Replace(input, match =>
            {
                if (match.Groups["roman"].Success)
                {
                    if (match.Value == "I")
                    {
                        return MatchComesAfterChapterOrEpisodeOrAtEndOfString(input, match) ? "1".PadLeft(numberLength, '0') : match.Value;
                    }
                    else if (match.Value == "X")
                    {
                        return MatchComesAfterChapterOrEpisodeOrAtEndOfString(input, match, maxDistanceFromEnd: 4) && !MatchComesBeforeDashAndWord(input, match)
                            ? "10".PadLeft(numberLength, '0')
                            : match.Value;
                    }
                ...
                {
                    return MatchComesAfterChapterOrEpisodeOrAtEndOfString(input, match)
                        ? numberWordValues[match.Value].ToString(new string('0', numberLength))
                        : match.Value;
                }
        ...
        {
            var matchIsAtEndOfString = MatchIsNearEndOfString(input, match, maxDistanceFromEnd);
            var theBitImmediatelyPriorToTheMatch = input.Substring(Math.Max(0, match.Index - 9), length: Math.Min(9, match.Index));
        ...
        private static bool MatchComesBeforeDashAndWord(string input, Match match)
        {
            ...
            var nextChar = input[match.Index + match.Length];
            if (nextChar != '-')
            {
                return false;
            }
            ...
            for (var i = match.Index + match.Length + 1; i < input.Length; i++)
            {
                var c = input[i];
        ...
        {
            var distance = input.Length - (match.Index + match.Length);
